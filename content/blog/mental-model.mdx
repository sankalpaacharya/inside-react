...
Now this raises an interesting question 🤔: how does the same React code run on such different platforms? A web app uses the DOM, a mobile
...
then this blog is for you. We will build a mental model for what React is, unwrapping small pieces step by step.
...
To understand how this works, let's first understand how React is structured
...
* **Reconciler** – the part that compares UI trees and decides what needs to change (diffing algorithm)
* **Host Instance** – the platform-specific renderer (DOM nodes, native views, etc.)
...
If you check the [React source code](https://github.com/facebook/react/tree/main/packages), you’ll see two separate packages: `react-reconciler` and `react-dom`.
...
Without React, you’d manually handle:
...
> “When state is `count`, the UI should look like this.”
