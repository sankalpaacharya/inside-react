...
because thats how browsers work, unless your callstack is empty browser can't push the click, keystroke events 
...
functions takes way too long to process, browser won't have the control to push those events in a callstack.
...
ut wait—if React exits the callstack, how does it remember where 
it was?

This is where Fiber's structure matters. React represents your 
component tree as a linked list of objects. Each object (a "fiber") 
has pointers to:
- Its first child
- Its next sibling  
- Its parent

So when React yields control, it just saves a pointer: "I was 
working on this fiber." When it comes back, it follows the pointers 
to find the next unit of work. No call stack needed—React is 
walking its own data structure.

To understand this better, let's look at the original design principles documented by Sebastian Markbåge (one of the core React team members):
